...
Implementado em 31/07/2026 (antes só valia como intenção, o código deixava editar por 8h):

- todo registro nasce como **Rascunho** (editável só por quem criou) ou já **Assinado** (atestado/receituário/relatório/laudo/pedido de exames, que não têm fluxo de rascunho próprio ainda);
- **Assinado é imutável** — bloqueado no banco por trigger, não só na tela; nem o autor, nem Administrador, conseguem mudar título/conteúdo/tipo depois;
- correção de um registro assinado vira uma **retificação**: registro novo, vinculado ao original, com motivo obrigatório — quem pode retificar é só quem assinou o original;
- a assinatura grava nome e CRM/COREN **no momento de assinar** (não é um link vivo pro cadastro, que pode ser corrigido depois).
...
| 31/07/2026 | Prontuário: registro assinado fica imutável (trigger no banco), correção vira retificação vinculada ao original; atendimento ganha Rascunho (editável, salvo no banco) e Finalizar e Assinar (grava nome+CRM/COREN no momento da assinatura); histórico passa a mostrar autoria, é agrupado por data, ganha filtro por tipo e busca por texto, e ações explícitas (Imprimir/Editar/Assinar/Retificar) em vez de depender só de duplo clique; tamanho dos painéis do prontuário passa a ser lembrado (localStorage); dados clínicos do topo ficam recolhíveis | Revisão externa apontou que o registro "consolidado" podia ser reescrito por 8h (o código chamava a variável de `dentroDE24h`, mas o prazo real era 8h) sem deixar rastro visível na tela — o mais importante dos apontamentos; RLS de UPDATE em `consultas` também foi restringida ao autor (ou Administrador), que antes deixava qualquer clínico editar registro de colega via API direta. Itens 2 (campos clínicos estruturados), 7 (cirurgias/medicamentos estruturados) e 8 (comorbidades em 4 estados) da mesma revisão ficaram de fora a pedido do usuário |
| 31/07/2026 | Migração `supabase_prontuario_assinatura.sql` corrigida: normaliza `status` antigo (`'Realizada'`/`NULL`) para `'Assinado'` antes de criar a constraint, e adiciona `NOT NULL` explícito | `consultas.status` já existia desde `supabase_schema.sql`, com `DEFAULT 'Realizada'`, nunca usado por nenhuma tela — `ADD COLUMN IF NOT EXISTS` foi um no-op em banco existente e a `CHECK` nova falhou (`23514`) contra linhas antigas |
| 31/07/2026 | Correção de bug: em "Dados Clínicos" do prontuário, erro ao salvar (Supabase fora do ar) fechava o modo de edição mesmo sem gravar, e não avisava o usuário — passou só a `console.error` | Mesmo padrão de risco já corrigido em outros pontos (rodada de 28/07): profissional pode acreditar que salvou uma alteração clínica que na verdade não foi para o banco |
| 31/07/2026 | Substituídos os ~40 `alert()` do app por um sistema central de notificação (`ToastProvider`/`useToast`, canto superior direito, não bloqueia a tela) | Pedido direto do usuário após revisão externa; `alert()` trava a interface inteira e é descartado pelos navegadores em iframes/PWA — mantido `window.confirm()` nas ações destrutivas (desativar paciente, excluir procedimento/convênio), que é um caso de uso diferente (confirmação, não notificação) |
| 31/07/2026 | `npm test` ganha mock manual do Supabase (`src/services/__mocks__/supabase.js`) e passa de 1 para 18 testes (guards de rota por função, `usePacienteAtual`, limpeza de rascunho no logout, salvar/preservar atendimento em caso de falha do servidor) | Pedido direto do usuário após revisão externa apontar que só havia um teste de "renderiza sem quebrar", com aviso de `act()` por falta de mock da sessão do Supabase |
