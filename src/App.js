...
const FUNCOES_FINANCEIRO = ['Financeiro', 'Administrador'];

function SomenteFinanceiro({ children }) {
  const { funcao } = useAuth();
...
function CatalogoFinanceiroPage() {
...
const COBRANCA_STATUS_CORES = { 'Pendente': '#f59e0b', 'Pago': '#28a745', 'Cancelado': '#6c757d', 'Glosado': '#ef4444' };

function NovaCobrancaForm({ pacientes, procedimentos, convenios, valoresConvenio, medicos, userId, onCriado, onCancelar }) {
  const [busca, setBusca] = React.useState('');
  const [pacienteId, setPacienteId] = React.useState('');
...
function FinanceiroPage({ pacientes }) {
  const { user, funcao } = useAuth();
